            ...
            <div class="tab-content p-t-15">
              <form id="saveConfigForm" action="./ajax_article.php?submit=save_config" method="POST">
                <style>
                  ...
                  .tab-content>form>.tab-pane.show.active {
                    display: block;
                  }
                </style>
    ...
    btn.addEventListener('click', function() {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          ...
          if (xhr.status === 200) {
            try {
              result.textContent = JSON.parse(xhr.responseText).msg;
            } catch (e) {
              result.textContent = '响应解析失败';
            }
          } else {
